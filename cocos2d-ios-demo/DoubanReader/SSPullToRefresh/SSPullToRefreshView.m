...
- (void)finishLoading {
	...
	__unsafe_unretained SSPullToRefreshView *blockSelf = self;
...
- (void)setState:(SSPullToRefreshViewState)state animated:(BOOL)animated expanded:(BOOL)expanded completion:(void (^)(void))completion {
    __unsafe_unretained SSPullToRefreshView *blockSelf = self;
    completion = ^{
        blockSelf.state = SSPullToRefreshViewStateNormal;
    };
    [self _setState:(SSPullToRefreshViewState)state animated:(BOOL)animated expanded:(BOOL)expanded completion:(void (^)(void))completion];
}
